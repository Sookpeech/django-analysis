...
def uploadTos3(wav_file_title, wav_file_path, count, user_id, rand):
    ...
    for i in range(count):
        ...
        audio = open(file, 'rb')
        try:
            upload = s3.Bucket(bucket_name).put_object(Key="{}_{}/{}_{}.wav".format(user_id, rand, wav_file_title, save_cnt), Body=audio)
            save_cnt+=1
        except:
...
def return_transcripts_async(saved_file_count, wav_file_title, user_id, rand):
    global transcripts
    ...
    for i in range(saved_file_count):
        transcribeWavFile(wav_file_title, i, transcribe, user_id, rand)
...
def transcribeWavFile(wav_file_title, count, transcribe, user_id, rand):
    # run transcribe
    job_uri = 'https://s3.ap-northeast-2.amazonaws.com/{}/{}_{}/{}_{}.wav'.format(bucket_name, user_id, rand, wav_file_title, count)
...
def deleteS3WavFile(user_id, rand):
    s3 = boto3.resource('s3')
    bucket = s3.Bucket(bucket_name)
    bucket.objects.filter(Prefix='{}_{}'.format(user_id, rand)).delete()
